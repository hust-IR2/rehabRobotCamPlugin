...
FTWrapper::FTWrapper()
{
	...
	mFTSensor.CreateDispatch(_T("ATIDAQFT.FTSensor"));
	ActiveCalibration = -1;
	status = UNKNOWN;
	daqerrmsg = "";
	bias = false;
...
int FTWrapper::LoadCalFile(CString filename)
{
	...
	CString filter = _T("ATI DAQ F/T Calibration Files (*.cal)|*.cal||");
	...
	if (filename == "") {
		...
		fileMenu.DoModal();*/
		//选择默认的校准量程文件
		filename =_T("D:\\RB\\project\\RB Robot\\RB Robot app\\RB-Robot-test-app\\motor test 20151013\\Calibration\\FT18258.cal");//fileMenu.GetPathName(); ////
	}
...
//	}
	...
	mFTSensor.SetCalFilePath((LPCTSTR)filename);
...
int FTWrapper::GetForcesAndTorques(double output[7])
{
	if (status == ALL_READY) {
		...
		COleSafeArray forces = GetActiveCalibration().ConvertToFT(voltages,(LPCTSTR)"lb",(LPCTSTR)"in-lb");
		for (i=0; i<6; i++) {
			// COleSafeArrays use pass-by-reference methods to read or write
...
void FTWrapper::AddToolTransform(CString axis, double Value, CString units)
{
	...
	if (status & CALFILE_LOADED) {
		...
		tools.Add((LPCTSTR)axis, Value, (LPCTSTR)units);
	}
}
